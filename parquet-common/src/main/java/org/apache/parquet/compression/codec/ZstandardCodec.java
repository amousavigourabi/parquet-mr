...
public class ZstandardCodec implements CompressionCodec {
  ...
  private boolean bufferPool = true;
  private int level = 3;

  @Override
  public InputStream createInputStream(ByteBufferInputStream inputStream) throws IOException {
    throw new CodecNotYetImplementedException("ZSTD decompression is not yet supported by this CompressionCodecFactory implementation.");
  }
  ...
  public void configureCodec(ParquetConfiguration configuration) {
    bufferPool = configuration.getBoolean(PARQUET_COMPRESS_ZSTD_BUFFERPOOL_ENABLED, bufferPool);
    level = configuration.getInt(PARQUET_COMPRESS_ZSTD_LEVEL, level);
  }
}
